/**
 * Serverless API Route: /api/leads
 * Busca todos os leads ao vivo da API do Supremo CRM e retorna no formato
 * esperado pelo dashboard.
 */
...
module.exports = async function handler(req, res) {
    ...
    if (!TOKEN) {
        res.status(500).json({ error: 'SUPREMO_CRM_TOKEN não configurado.' });
        return;
    }
    ...
    try {
        ...
        console.error('[api/leads] Erro ao buscar dados do Supremo CRM:', err.message);
        res.status(502).json({ error: 'Falha ao buscar dados do Supremo CRM.', detail: err.message });
    }
};
